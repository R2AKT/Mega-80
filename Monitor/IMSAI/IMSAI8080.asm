...
;;
; Based on IMSAIGuy 8080-monitor (https://github.com/imsaiguy/8080-Monitor)
; Adapted for ZASM assembler and 'Mega-80(580)' project by R2AKT 17/04/2026 (https://github.com/R2AKT)
...
#if MemSize = 32
CORE   	EQU     7FFFH           ; TOP OF UTILITY RAM, 32k version
#else
CORE    EQU     1FFFH           ; TOP OF UTILITY RAM, 8k version
#endif
...
;*********************************************************
;
#code 	MAIN, ROM, *			; START OF MONITOR READ-ONLY-MEMORY
;
...
;
		LXI     H,PCLOC	        ;PLACE DEBUG ENTRANCE AND EXIT TEMPLATE
...
GETCH:  ;PUSH	H				;SAVE REGISTERS
        ;PUSH	D
        ;PUSH	B
CIN:	IN		USART8251CMD	;GET STATUS OF CONSOLE 
		ANI		USART_ST_RxRdy	;CHECK FOR RECEIVER BUFFER READY 
		JZ		CIN				;WAIT till recieved 
		IN		USART8251DATA	;GET CHARACTER 
...
;
;
POLL:   IN 		USART8251CMD	;GET STATUS OF CONSOLE 
		ANI 	USART_ST_RxRdy	;CHECK FOR RECEIVER BUFFER READY 
        RET                     ;RETURN
;
;
...
COUT:	IN 		USART8251CMD	;GET STATUS OF CONSOLE 
		ANI 	USART_ST_TxRdy	;SEE IF TRANSMITTER READY 
		JZ  	COUT			;NO - WAIT till ready
		POP     PSW				; Restore A from stack
		OUT 	USART8251DATA	;SEND Character TO CONSOLE 
		RET
...
;*********************************************************
;
M0:     DB      CR,LF,"8080 SYSTEM MONITOR",CR,LF
		DB		"ADAPTED FOR MEGA580 BY R2AKT",CR,LF+80H
...
;
#code 	MON_RAM, RAM,*			;SOME RAM SCRATCHPAD AREA                  
;
; SYSTEM RAM AREA
;
